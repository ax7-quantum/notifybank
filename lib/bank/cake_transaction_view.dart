...
class CakeTransactionView extends StatefulWidget {
  const CakeTransactionView({Key? key}) : super(key: key);

  @override
  State<CakeTransactionView> createState() => _CakeTransactionViewState();
}

class _CakeTransactionViewState extends State<CakeTransactionView> {
  ...
  void initState() {
    super.initState();
    _loadCakeTransactions();
  }

  // Tải danh sách giao dịch Cake từ SharedPreferences
  Future<void> _loadCakeTransactions() async {
    setState(() {
      _isLoading = true;
    });
    
    try {
      final String transactionsJson = await _channel.invokeMethod('getCakeTransactions');
      ...
      try {
        ...
        print("Lỗi khi parse JSON giao dịch Cake: $e");
      ...
      print("Lỗi khi tải giao dịch Cake: ${e.message}");
      setState(() {
        _isLoading = false;
      });
    }
  ...
  // Xóa tất cả giao dịch Cake
  Future<void> _clearCakeTransactions() async {
    try {
      await _channel.invokeMethod('clearCakeTransactions');
      _loadCakeTransactions();
    } on PlatformException catch (e) {
      print("Lỗi khi xóa giao dịch Cake: ${e.message}");
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Log để debug
    print("Số lượng giao dịch Cake: ${_transactions.length}");
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  Text(
                    'Danh sách giao dịch Cake (${_transactions.length})',
                  ...
                  if (_transactions.isNotEmpty)
                    TextButton.icon(
                      onPressed: _clearCakeTransactions,
            ...
            Container(
              ...
              child: _isLoading
                  ...
                  : _transactions.isEmpty
                      ? const Center(
                          child: Text(
                            'Không có giao dịch Cake nào',
                            style: TextStyle(fontSize: 16),
                          ),
                        )
                      : RefreshIndicator(
                          onRefresh: _loadCakeTransactions,
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              String content = transaction['content']?.toString() ?? 'Không có nội dung';
